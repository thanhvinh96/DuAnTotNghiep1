...
const MedicalForm = ({ onSubmit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://ehrmedical.online/api/medicaldata/add",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...
            <Card className="p-3 mt-3">
              ...
              <Row>
                <Col md={6}>
                  ...
                  <p>
                    <strong>Ngày sinh:</strong> {patientInfo.birthday}
                  </p>
                  <p>
                    <strong>Địa chỉ:</strong> {patientInfo.address}
                  </p>
                  <p>
                    <strong>Số BH:</strong> {patientInfo.sobh}
                  </p>
                  <p>
                    <strong>Token Medical:</strong> {patientInfo.tokenmedical}
                  </p>
                </Col>
                <Col md={6}>
                  <p>
                    <strong>Giới tính:</strong> {patientInfo.sex}
                  </p>
                  <p>
                    <strong>Cân nặng:</strong> {patientInfo.weight} kg
                  </p>
                  <p>
                    <strong>Chiều cao:</strong> {patientInfo.height} cm
                  </p>
                  <p>
                    <strong>Email:</strong> {patientInfo.email}
                  </p>
                  <p>
                    <strong>Số điện thoại:</strong> {patientInfo.phoneNumber}
                  </p>
                </Col>
              </Row>
              <Row className="mt-3">
                  ...
                  <Form onSubmit={handleSubmit}>
                    <Table className="table table-bordered text-center mt-5">
                      ...
                      <tbody>
                        ...
                        {Object.entries(patientInfo.fieldsToShare).map(
                          ([key, value], index) => (
                            <tr key={index}>
                              <td>{index + 1}</td>
          ...
          <div className="mt-4">
            ...
            <Col md={12}>
              <Form.Group className="mb-3">
                ...
                <FileUploader
                  onFileUpload={(files) =>
                    handleFileUpload(files[0], "ImagesMedical")
                  }
                />
              </Form.Group>
            </Col>
            ...
            <Row className="mb-3">
              ...
              <Col md={6}>
                ...
                <Form.Control
                  ...
                  onChange={(e) =>
                    handleSymptomChange("conclusion", e.target.value)
                  }
                />
              </Col>
            </Row>
      ...
      {addedData && (
        <Card className="p-3 mt-4">
          ...
          <Table className="table table-bordered text-center mt-3">
            ...
            <tbody>
              {addedData.exam_records &&
                addedData.exam_records.map((item, index) => (
                  <tr key={index}>
                    <td>{index + 1}</td>
          ...
          <div className="mt-3">
            <p>
              ...
              {addedData.diagnosis_info?.symptom || "N/A"}
            </p>
            <p>
              <strong>Kết luận:</strong>{" "}
              {addedData.diagnosis_info?.conclusion || "N/A"}
            </p>
            <p>
              <strong>CCCD:</strong> {addedData.patient_cccd || "N/A"}
            </p>
          </div>
        </Card>
      )}
