...
const CreateAppointment: React.FC = () => {
  ...
  const [typeclinics, setTypeclinics] = useState<
  ...
  const [dataDepartByBranch, setdataDepartByBranch] = useState<
    { _id: string; departmentName: string }[]
  >([]);
  ...
  const showdataDepartByBranch =
    dataDepartByBranch.length > 0
      ? dataDepartByBranch
  ...
  const filteredAppointments = appointments.filter((appointment) =>
    appointment.branch.includes(searchBranch)
  );
